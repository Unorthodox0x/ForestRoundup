...
import type { TerrainSpriteName, TileObjects } from '@/types';


describe('initializes chunk class with correct coordinates', () => {
	test('first chunk', () => {
		const chunk = new Chunk(0);
		// console.log('first chunk coords', chunk.coords)
	...
	test('last chunk', () => {
		...
		// console.log('last chunk coords', chunk.coords)
...
describe('initialize', ()=> {
	...
	describe('chunk tiles initialize', () => { 
		...
		test('sets coordinates of chunk tiles', () => {
			const chunk = new Chunk(0);
