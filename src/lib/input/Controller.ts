import { EnemyDirection, PlayerDirection } from "@/types";
import Enemy from "../chars/Enemy";
import Player from "../chars/Player";
import { enemyDown, enemyLeft, enemyRight, enemyUp, playerDown, playerLeft, playerRight, playerUp } from "@/constants/input";
...
export default class Controller {
	 ...
	 * */
	static async moveUp(char: Player|Enemy){
		/// to move up, decrement values closer to 0
		char.canvasY -= gameSquare/2
		await char.draw(renderStateTwo) /// begin move
		setTimeout(async()=> {
			char.canvasY -= gameSquare/2
			await char.draw(renderStateOne) /// finish move
		}, 100);
	}
	static async moveDown(char: Player|Enemy){
	...
	static async moveLeft(char: Player|Enemy){
	...
	static async moveRight(char: Player|Enemy){
		char.canvasX += gameSquare/2
		await char.draw(renderStateTwo) /// begin move
		setTimeout(async()=> {
			char.canvasX += gameSquare/2
