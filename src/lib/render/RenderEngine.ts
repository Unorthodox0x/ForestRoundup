...
class RenderEngine {
	...
	async drawTerrain(
		...
		const renderChunks = board.chunks.flatMap(chunk =>{
			if(chunk.tiles.length < 1) return;
			new Promise((resolve) => { 
				const promises = chunk.tiles.map( 
					async (tile) =>{
						...
						return await this.drawSprite(context, sprite, renderState, { x: canvasX, y: canvasY }) /// only one render state
					})

				Promise.all(promises).then(()=>{
					resolve(undefined) /// execute all 'render terrain' events in chunk batch
				});
			})
		});

		await Promise.all(renderChunks);
	}
	...
	async drawTrees(	
		...
		const renderChunks = board.chunks.flatMap(chunk => {
			...
			new Promise((resolve) => { 
				...
				Promise.all(promises).then(()=>{
					resolve(undefined) /// execute all 'render tree' events in chunk batch
				});
			})
		});
			...
			new Promise((resolve) => { 
				const promises = chunk.tiles.map(
					async tile =>{
						...
						const canvasY = chunkHeight * chunk.coords.y * gameSquare + tile.coords.y * gameSquare
													/// rocks only have 1 state
						return await this.drawSprite(context, sprite, renderStateOne, {x: canvasX, y: canvasY}) /// only one render state
					});
